...
# 创建随机解集、两个种群交换信息、过程中会有信息变异，去除劣势的解(优胜劣汰)
...
def select(pop, fitness): # 关于pop适应度的自然选择
    # replace=True表示可以取相同数字，p=(fitness) / (fitness.sum())表示选取元素的概率
    # 概率是不断叠加的，最后为1
...
if __name__ == "__main__":
    fig = plt.figure()
    ax = Axes3D(fig) # 这两步是建立画布
    plt.ion() # 将画图模式改为交互模式，程序遇到plt.show不会暂停，而是继续执行
    plot_3d(ax) #plot_3d函数生成数据并绘图

    pop = np.random.randint(2, size=(POP_SIZE, DNA_SIZE * 2))
    for _ in range(N_GENERATIONS): # 迭代N代
        x, y = translateDNA(pop) # 编码，产生初始群体
        if 'sca' in locals(): # locals()函数返回字典类型的局部变量
            sca.remove() # remove()函数用于清空函数图像
        sca = ax.scatter(x, y, F(x, y), c='black', marker='o')
        plt.show()
        print(10000)
        plt.pause(0.1) # scatter特征值散点图
